...
FIRST_CASE="case 'academy_summary':"

[ -f "$ROUTES_FILE" ] || exit 1
EXISTING=$(grep -c "case 'academy_summary'" "$ROUTES_FILE" 2>/dev/null || echo "0")
...
NEW=$(grep -c "case 'academy_summary'" /tmp/routes_patched_${MODULE}.js)
[ "$NEW" = "1" ] || exit 1
...
echo "✅ mega_stubs 293 cases inserted | Backup: $BACKUP_FILE"
