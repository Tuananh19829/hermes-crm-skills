...
FIRST_CASE="case 'search_products':"

[ -f "$ROUTES_FILE" ] || exit 1
EXISTING=$(grep -c "case 'search_products'" "$ROUTES_FILE" 2>/dev/null || echo "0")
...
NEW=$(grep -c "case 'search_products'" /tmp/routes_patched_${MODULE}.js)
[ "$NEW" = "1" ] || exit 1
sudo mv /tmp/routes_patched_${MODULE}.js "$ROUTES_FILE"
docker restart appcrm-api
sleep 4
for s in search_products get_stock_report create_import_stock create_export_stock create_stock_transfer create_stocktake manage_suppliers create_product manage_brands manage_units manage_fixed_assets; do
...
echo "✅ inventory 11 LIVE | Backup: $BACKUP_FILE"
