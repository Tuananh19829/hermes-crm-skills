...
FIRST_CASE="case 'search_services':"

[ -f "$ROUTES_FILE" ] || exit 1
EXISTING=$(grep -c "case 'search_services'" "$ROUTES_FILE" 2>/dev/null || echo "0")
...
NEW=$(grep -c "case 'search_services'" /tmp/routes_patched_${MODULE}.js)
[ "$NEW" = "1" ] || exit 1
sudo mv /tmp/routes_patched_${MODULE}.js "$ROUTES_FILE"
docker restart appcrm-api
sleep 4
for s in search_services get_service_detail list_service_categories search_treatments create_treatment cancel_service_refund manage_marketing_cost; do
...
echo "✅ services 7 LIVE | Backup: $BACKUP_FILE"
