...
FIRST_CASE="case 'cashbook_report':"

[ -f "$ROUTES_FILE" ] || exit 1
EXISTING=$(grep -c "case 'cashbook_report'" "$ROUTES_FILE" 2>/dev/null || echo "0")
...
NEW=$(grep -c "case 'cashbook_report'" /tmp/routes_patched_${MODULE}.js)
[ "$NEW" = "1" ] || exit 1
sudo mv /tmp/routes_patched_${MODULE}.js "$ROUTES_FILE"
docker restart appcrm-api
sleep 4
for s in cashbook_report create_cashbook_entry list_debts collect_debt list_service_cards deposit_card list_input_invoices manage_payment_methods manage_finance_categories; do
...
echo "✅ finance 9 LIVE | Backup: $BACKUP_FILE"
